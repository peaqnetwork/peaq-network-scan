...
export default function ExtrinsicSnapshot({ extrinsicId }) {
  const blockNumber = extrinsicId.split("-")[0];
  const extrinsicIndex = extrinsicId.split("-")[1];

  const [block, setBlock] = useState(null);
  const [extrinsic, setExtrinsic] = useState(null);
  const [isFetchingExtrinsic, setIsFetchingExtrinsic] = useState(false);

  const { api } = useSubstrateState();
  useEffect(() => {
    let isMounted = true;
    setIsFetchingExtrinsic(true);
    const getBlock = async () => {
      try {
        ...
        const extrinsicEvent = allEvents.filter(
          ({ phase }) =>
        ...
        if (extrinsic) {
          extrinsic.isSuccessfull = api.events.system.ExtrinsicSuccess.is(
            extrinsicEvent[0]?.event
          );
        }
        if (isMounted) {
          setBlock(blockObj);
          setExtrinsic(extrinsic);
          setIsFetchingExtrinsic(false);
        }
      } catch (err) {
        console.error(err);
        setIsFetchingExtrinsic(false);
  ...
  if (block && Number(extrinsicIndex) >= block.extrinsics.length) {
    return (
      <div className="mt-40">
        <InfoPlaceholder text="Sorry! That extrinsic does not exist" />
      </div>
    );
  }

  if (isFetchingExtrinsic || !extrinsic) {
    return (
      <div className="mt-40">
        <InfoPlaceholder text="Loading extrinsic..." />
      </div>
    );
  }

  const extrinsicObj = block?.extrinsics[extrinsicIndex];

  return (
    <div className="block-snapshot above-footer">
      <table className="table">
        <tbody>
          <tr>
            <td className="block-snapshot-title">Time</td>
            <td className="text-dark-white">
              {new Date(getBlockTime(block))
                .toISOString()
                .slice(0, 19)
                .replace("T", " ")}{" "}
              (+UTC)
          ...
          <tr>
            <td className="block-snapshot-title">Block</td>
            <td>
              <span className="d-flex align-items-center">
                ...
                <span className="ml-3 text-accent-purple">
          ...
          <tr>
            <td className="block-snapshot-title">Life Time</td>
            <td className="text-dark-white">
              {extrinsic?.era.isImmortalEra ? "immortal" : "mortal"}
            </td>
          </tr>
          <tr>
            <td className="block-snapshot-title">Extrinsic Hash</td>
            <td className="text-dark-white d-flex">
              <span className="mr-3">{extrinsic?.hash.toHex()}</span>{" "}
              <span className="ml-3">
                <svg
                  width="24"
                  height="24"
                  ...
                  className="mr-3"
                  cursor="pointer"
                  onClick={() => copyText(extrinsic?.hash.toHex())}
                >
                  <path
          ...
          <tr>
            <td className="block-snapshot-title">Module</td>
            <td className="text-dark-white">
          ...
          <tr>
            <td className="block-snapshot-title">Call</td>
            <td className="text-dark-white">
              <span className="button-outline">{extrinsic?.method.method}</span>
            </td>
          </tr>

          <tr>
            <td className="block-snapshot-title">Sender</td>
            <td className="text-accent-purple">
              {extrinsic?.isSigned && (
                <span className="mr-3">{extrinsicObj?.signer?.Id} </span>
              )}
              {extrinsic?.isSigned && (
                <span className="ml-3">
                  <svg
                    ...
                    className="mr-3"
                    cursor="pointer"
                    onClick={() => copyText(extrinsicObj?.signer.Id)}
                  >
                    <path
                      d="M17.9091 19.3636H8.90909V7.90909H17.9091V19.3636ZM17.9091 6.27273H8.90909C8.4751 6.27273 8.05888 6.44513 7.75201 6.75201C7.44513 7.05888 7.27273 7.4751 7.27273 7.90909V19.3636C7.27273 19.7976 7.44513 20.2138 7.75201 20.5207C8.05888 20.8276 8.4751 21 8.90909 21H17.9091C18.3431 21 18.7593 20.8276 19.0662 20.5207C19.3731 20.2138 19.5455 19.7976 19.5455 19.3636V7.90909C19.5455 7.4751 19.3731 7.05888 19.0662 6.75201C18.7593 6.44513 18.3431 6.27273 17.9091 6.27273ZM15.4545 3H5.63636C5.20237 3 4.78616 3.1724 4.47928 3.47928C4.1724 3.78616 4 4.20237 4 4.63636V16.0909H5.63636V4.63636H15.4545V3Z"
                      fill="#979798"
          ...
          <tr>
            ...
            <td>
              <span className="d-flex align-items-center">
                ...
                <span className="ml-3 text-dark-white">
                  {extrinsic?.isSuccessfull ? "Success" : "Failed"}
                </span>
              </span>
            </td>
          </tr>
          <tr>
            ...
            <td className="text-dark-white d-flex align-items-center ">
              <span className="mr-3">
                {extrinsicObj &&
                  getExtrinsicParameters(extrinsicObj?.method.args)}
              </span>
              <span className="ml-40">
                <svg
                  ...
                  cursor="pointer"
                  onClick={() =>
                    copyText(getExtrinsicParameters(extrinsicObj?.method.args))
                  }
                >
                  <path
                    d="M17.9091 19.3636H8.90909V7.90909H17.9091V19.3636ZM17.9091 6.27273H8.90909C8.4751 6.27273 8.05888 6.44513 7.75201 6.75201C7.44513 7.05888 7.27273 7.4751 7.27273 7.90909V19.3636C7.27273 19.7976 7.44513 20.2138 7.75201 20.5207C8.05888 20.8276 8.4751 21 8.90909 21H17.9091C18.3431 21 18.7593 20.8276 19.0662 20.5207C19.3731 20.2138 19.5455 19.7976 19.5455 19.3636V7.90909C19.5455 7.4751 19.3731 7.05888 19.0662 6.75201C18.7593 6.44513 18.3431 6.27273 17.9091 6.27273ZM15.4545 3H5.63636C5.20237 3 4.78616 3.1724 4.47928 3.47928C4.1724 3.78616 4 4.20237 4 4.63636V16.0909H5.63636V4.63636H15.4545V3Z"
