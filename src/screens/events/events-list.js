...
import PaginationForArchive from "../../components/pagination/pagination-for-archive";
...
export default function EventsList({
  ...
  if (isLoadingEvents) {
    return (
      <InfoPlaceholder text="Loading events list..." isListContainer isBare />
    );
  }
  if (events.length < 1) {
    return (
              ...
              ({
                ...
                <tr key={eventId}>
                  ...
                  <td className="text-dark-white">
                    {formatTime(new Date(created_at)).fromNow()}
                  </td>
